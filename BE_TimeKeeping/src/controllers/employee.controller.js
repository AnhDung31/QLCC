...
// Cập nhật nhân viên
  ...
  handleCheckinSave: async (processedData) => {
    try {
      const faceIdForCheckin = processedData.data.faceBase64 || 'N/A';
  ...
  handleRegistrationSave: async (registrationData) => {
    try {
      ...
      // Nếu chưa có userId, tự động tìm admin quản lý thiết bị
      if (!registrationData.userId && registrationData.deviceId) {
        const admin = await User.findOne({ role: 'admin', devices: registrationData.deviceId });
        if (admin) {
          registrationData.userId = admin._id;
        }
      }
      ...
      const newEmployee = await employeeService.createEmployee({
        ...
        userId: registrationData.userId, // Đã được gán ở trên nếu tìm thấy admin
